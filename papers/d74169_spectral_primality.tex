...
\subsection{Discovery: $\phi^n$ Aligns with Primes}

\begin{theorem}[Golden Ratio Enrichment]
When $\lfloor \phi^n \rfloor$ lands on or near a prime, there is $1.73\times$ enrichment over random expectation ($\chi^2$ test $p < 0.001$).
\end{theorem}

where $\phi = (1 + \sqrt{5})/2 \approx 1.618$.

\subsection{Lucas Numbers are Prime-Enriched}

Lucas numbers $L_n = \phi^n + \psi^n$ (where $\psi = (1 - \sqrt{5})/2$) have special spectral properties:
...
\textbf{Separation} & \textbf{0.0364} & Significant \\
\bottomrule
\end{tabular}
\caption{Lucas positions show prime-like spectral signature}
\end{table}

Lucas primes: $L_2 = 3$, $L_4 = 7$, $L_5 = 11$, $L_7 = 29$, $L_8 = 47$, \ldots

\subsection{The $\phi$-Prime Ladder}

Starting from small primes, computing $p \times \phi$ and rounding to nearest prime:
\[
2 \to 3 \to 5 \to 8 \to 13 \to 21 \to 34 \to 55 \to 89 \to 144 \to \ldots
\]

The ratio of consecutive elements in the ladder converges to $\phi$.

\begin{proposition}
The golden ratio $\phi$ is spectrally entangled with the prime distribution.
\end{proposition}
...
\begin{enumerate}
    ...
    \item \textbf{Golden Ratio:} $\phi^n \to$ prime alignment at $1.73\times$ enrichment; Lucas primes
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Ribenboim, P. ``The Little Book of Bigger Primes.'' Springer (2004). [Lucas primes and Fibonacci sequences]
\end{enumerate}
